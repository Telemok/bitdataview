...
export class BitDataView {
	...
	setAt_Boolean(bitIndexAt, value) {
		value = valda.boolean.assert(value, `BitDataView.setAt_Boolean(wrong value)`);
	 ...
	 * @description
	 */
	_setUntil_Bool_noAsserts(bitIndexUntilEnd, value) {
		this._setBitInMemoryAddress_noAsserts(value, this[_countBitsPushed] - 1 - bitIndexUntilEnd);
	}
	...
	setUntil_Boolean(bitIndexUntilEnd, value) {
		bitIndexUntilEnd = valda.integerMinMax.assert(bitIndexUntilEnd, 0, this.getStoredBits() - 1, `BitDataView.setUntil_Boolean(wrong bitIndexUntilEnd)`);
		value = valda.boolean.assert(value, `BitDataView.setUntil_Boolean(wrong value)`);
		this._setUntil_Bool_noAsserts(bitIndexUntilEnd, value);
	}
	...
	getAt_Boolean(bitIndexAt) {
		valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - 1, `BitDataView.getAt_Boolean(bitIndex: ${bitIndexAt}, bitSize: ${this.getStoredBits()})`);
	...
	getUntil_Boolean(bitIndexUntil) {
		valda.integerMinMax.assert(bitIndexUntil, 0, this.getStoredBits() - 1, `BitDataView.getAt_Boolean(bitIndex: ${bitIndexUntil}, bitSize: ${this.getStoredBits()})`);
	...
	push_Booleans(value, count = 1) {
		value = valda.boolean.assert(value, `BitDataView.push_Booleans(wrong value)`);
		count = valda.integerMinMax.assert(count, 0, this.getAvailableBitsToPush(),`BitDataView.push_Booleans(wrong count)`);
	 ...
	 */
	unshift_Booleans(value, count = 1) {
		value = valda.boolean.assert(value, `BitDataView.unshift_Booleans(wrong value)`);
		count = valda.integerMinMax.assert(count, 0, this.getAvailableBitsToUnshift(),`BitDataView.unshift_Booleans(wrong count)`);
	...
	pop_Boolean() {
		if (this.getStoredBits() < 1)
			throw new Error(`BitDataView.pop_Boolean() error because no data available in buffer`);
	...
	shift_Boolean() {
		if (this.getStoredBits() < 1)
			throw new Error(`BitDataView.shift_Boolean() error because no data available in buffer`);
	...
	{
		countBitsToGet = valda.integerMinMax.assert(countBitsToGet, 0, 8, `BitDataView.getAt_Uint8orLess(wrong countBitsToGet)`);
		bitIndexAt = valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - countBitsToGet, `BitDataView.getAt_Uint8orLess(wrong bitIndexAt)`);
	 ...
	 * @return {number} - byteData
	 */
	pop_Uint8orLess(countBitsToPop)//countBitsToPop >0 <8
	{
	 ...
	 * @return {number} - byteData
	 */
	shift_Uint8orLess(countBitsToShift)//countBitsToPop >0 <8
	{
	...
	setAt_Int53orLess(bitIndexAt = 0, countBitsToSet = 54, value) {
	 ...
	 */
	setAt_Uint53orLess(bitIndexAt, countBitsToSet, value) {
		countBitsToSet = valda.integerMinMax.assert(countBitsToSet, 0, 53, `BitDataView.setAt_Uint53orLess(wrong countBitsToSet)`);
		bitIndexAt = valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - countBitsToSet, `BitDataView.setAt_Uint53orLess(wrong bitIndexAt)`);
	 ...
	 * @description
	 */
	push_Uint53orLess(countBitsToPush, value) {
	 ...
	 * @param {number} value - unsigned integer number to unshift. From 0 to 1FFFFFFFFFFFFF (2 ^ 53 - 1). Asserted.
	 */
	unshift_Uint53orLess(countBitsToUnshift, value) {
	 ...
	 * @return {number} - value
	 */
	getAt_Uint53orLess(bitIndexAt = 0, countBitsToGet = 53) {
		countBitsToGet = valda.integerMinMax.assert(countBitsToGet, 0, 53, `BitDataView.getAt_Uint53orLess(wrong countBitsToGet)`);
		bitIndexAt = valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - countBitsToGet, `BitDataView.getAt_Uint53orLess(wrong bitIndexAt)`);
	 ...
	 * @return {number} - value
	 */
	getAt_Int53orLess(bitIndexAt = 0, countBitsToGet = 54) {
	...
	pop_Uint53orLess(countBitsToPop)
	{
	...
	shift_Uint53orLess(countBitsToShift)
	{
	 ...
	 * @param {BigInt} value -
	 */
	setAt_BigUint64orLess(bitIndexAt = 0, countBitsToSet = 64, value)
	{
		//console.log("setAt_BigUint64orLess")
		countBitsToSet = valda.integerMinMax.assert(countBitsToSet, 0, 64/*, `BitDataView.setAt_BigUint64orLess(wrong countBitsToGet)`*/);
		bitIndexAt = valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - countBitsToSet/*, `BitDataView.setAt_BigUint64orLess(wrong bitIndexAt)`*/);
		//TODO check value
	 ...
	 * @description
	 */
	push_BigUint64orLess(countBitsToPush, value) {
...
/** get (take from custom plase of bitDataView) unsigned integer.
	 ...
	 * @param {number} countBitsToGet - Count bits from 0 to 64 of value to get. Not asserted.
	 */
	getAt_BigUint64orLess(bitIndexAt = 0, countBitsToGet = 64){
	...
	// 	for (let i = countBitsToUnshift - 1; i >= 0; i--)
	// 		this.unshift_Boolean((value >> i) & 1);
	// }
	// /**  unshift (add to left of bitDataView) unsigned integer.
	//  * @param {number} countBitsToUnshift - Count bits from 0 to 32 of value to push. Asserted.
	//  * @param {number} value - unsigned integer number to unshift. From 0 to 0xFFFFFFFF (2 ^ 32 - 1). Asserted.
	...
	// 		this.unshift_Boolean(value & mask);
	// }
	...
	// 		this.unshift_Boolean(((value >> i) & 1n) != 0n);
	// }
