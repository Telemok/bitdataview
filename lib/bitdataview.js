...
export class BitDataView{
	...
	constructor(parameters = {}) {
		...
		if('startOffsetBits' in parameters)
		{
			let startOffsetBits = valda.numberMinMax.parse(parameters, 'startOffsetBits', 0, this[_countBitsPushLimit] - 1);
			this[_countBitsPushed] = this[_countBitsShifted] = startOffsetBits;
		}
	}
	...
	{
		this[_countBitsPushed] += countBits;
	}
	push_Nothing(countBits)
	{
		countBits = valda.integerMinMax.assert(countBits, 0, this.getAvailableBitsToPush());
		this._push_Nothing_noAsserts(countBits);
	}
	_unshift_Nothing_noAsserts(countBits)
	{
		this[_countBitsShifted] += countBits;
	}
	...
	_setAt_Uint53orLess_noAsserts(bitIndexAt = 0, countBitsToSet = 53, value/*,  littleEndian = false*/)
	{
		...
		{
			//console.log(`setAt(${bitIndexAt}).LE.Uint${countBitsToSet}(${value})`);
		...
		{
			//console.log(`setAt(${bitIndexAt}).BE.Uint${countBitsToSet}(${value})`);
	...
	_setAt_Int53orLess_noAsserts(bitIndexAt = 0, countBitsToSet = 54, value)
	{
		...
		if(this.endianness_isLittleEndian())//littleEndian = true in C++ TelemokBitDataView
		{
			//console.log(`setAt(${bitIndexAt}).LE.Int${countBitsToSet}(${value})`);
			for(; countBitsToSet > 0;)//TO DO del countBitsToSet, use bitIndexAt in for
			{
				let subBitsCount = Math.min(countBitsToSet, 8);
				this._setAt_Uint8orLess_noAsserts(bitIndexAt ,subBitsCount, value);
		...
		{
			//console.log(`setAt(${bitIndexAt}).BE.Int${countBitsToSet}(${value})`);
			for(let index = 0; countBitsToSet > 0; index += 8)
			{
				let subBitsCount = Math.min(countBitsToSet, 8);
				this._setAt_Uint8orLess_noAsserts(bitIndexAt + countBitsToSet - subBitsCount, subBitsCount, value);
	...
	push_Int(countBitsToPush, value) {
		...
		value = valda.integerMinMax.assert(value, - (2 ** (countBitsToPush)), 2 ** (countBitsToPush) - 1);
		this.expandRightIfNeed(countBitsToPush);
	...
	_getAt_Int53orLess_noAsserts(bitIndexAt = 0, countBitsToGet = 54, /*TODO вернуть littleEndian, иначе не будет работать custom float*/)
	{
		...
		let sign = (Math.floor(result / (2 ** (countBitsToGet - 2)))&1) ? true : false;
	 ...
	 * @param {BigInt} value -
	 */
	_setAt_BigInt64orLess_noAsserts(bitIndexAt = 0, countBitsToSet = 64, value)
	{
		if(this.endianness_isLittleEndian())//littleEndian = true in C++ TelemokBitDataView
		{
			for(; countBitsToSet > 0; )
	...
	/** get (take from custom plase of bitDataView) unsigned integer.
	 * @param {number} bitIndex - Bit index from begin of bitDataView.
	 * @param {number} countBitsToSet - Count bits from 0 to 64 of value to get.
	 * @param {BigInt} value -
	 */
	...
	/** get (take from custom plase of bitDataView) signed integer.
	 * @param {number} bitIndex - Bit index from begin of bitDataView.
	 * @param {number} countBitsToSet - Count bits from 0 to 64 of value to get.
	 * @param {BigInt} value -
	 */
	setAt_BigInt(bitIndexAt = 0, countBitsToSet = 64, value)
	{
		//console.log("setAt_BigUint64orLess")
		countBitsToSet = valda.integerMinMax.assert(countBitsToSet, 0, 64/*, `BitDataView.setAt_BigUint(wrong countBitsToGet)`*/);
		bitIndexAt = valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - countBitsToSet/*, `BitDataView.setAt_BigUint(wrong bitIndexAt)`*/);
		//TODO check value
		return this._setAt_BigInt64orLess_noAsserts(bitIndexAt, countBitsToSet, value);
	}
	...
	_getAt_BigInt64orLess_noAsserts(bitIndexAt = 0, countBitsToGet = 64){
		...
		let sign = (maskSign & result) ? true : false;
		//console.log("sign",sign,maskSign,result)
		//console.log(`getAmask  ${maskNegative.toString(2)}`);
...
/** get (take from custom plase of bitDataView) signed integer.
	 ...
	 * @param {number} countBitsToGet - Count bits from 0 to 64 of value to get. Not asserted.
	 */
	getAt_BigInt(bitIndexAt = 0, countBitsToGet = 64){
		countBitsToGet = valda.integerMinMax.assert(countBitsToGet, 0, 64);
		bitIndexAt = valda.integerMinMax.assert(bitIndexAt, 0, this.getStoredBits() - countBitsToGet);
